...
const Report: React.FC<ReportProps> = ({ data }) => {
  ...
  const countIssuesBySeverity = (results: any[]) => {
    const counts = { critical: 0, high: 0, medium: 0, low: 0, warning: 0, note: 0 };
    
    results.forEach(result => {
      counts[result.severity as keyof typeof counts] = (counts[result.severity as keyof typeof counts] || 0) + 1;
    });
  ...
  // Function to render a results table
  const renderResultsTable = (results: any[], title: string) => {
          ...
          <tbody>
            {results.map((result, index) => (
              <tr key={index}>
                <td>{result.id}</td>
                <td>{result.name}</td>
                <td className={`severity-${result.severity}`}>
    ...
    <html lang="en">
      ...
      <body>
        ...
        <div className="tool-section">
          ...
          <div className="github-dependency">
